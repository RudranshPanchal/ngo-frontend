...
import VerificationPendingDashboard from '../components/VerificationPendingDashboard.jsx';
...
const DashboardPage = () => {
  ...
  const loadRealTimeData = async () => {
    try {
      ...
      const response = await fetch(import.meta.env.VITE_BACKEND_URL + '/api/auth/getAdminDashboard', {
        headers: {
      ...
      const activityResponse = await fetch(import.meta.env.VITE_BACKEND_URL + '/api/auth/getRecentActivity', {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  // 🚨 FORCE verification screen (ignores activeTab)
  if (isVolunteer && !isVerified && activeTab !== 'volunteer-profile') {
    return (
                      ...
                      <Card key={program.id} className="border-0 shadow-sm">
                        <CardContent className="p-6">
                          ...
                          <div className="flex justify-between items-center">
                            <span className={`text-xs px-2 py-1 rounded ${program.status === 'active' ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800'
                              }`}>
                              {program.status}
                            </span>
                ...
                <Card key={index} className="border-0 shadow-sm hover:shadow-md transition-shadow cursor-pointer">
                  <CardContent className="p-3 sm:p-4 lg:p-6">
                    <div className="flex items-center justify-between mb-3 sm:mb-4">
                      ...
                      <div className={`flex items-center gap-1 text-xs sm:text-sm ${stat.trendUp ? 'text-green-600' : 'text-red-600'
                        }`}>
                        <ArrowUpRight className={`h-3 w-3 sm:h-4 sm:w-4 ${stat.trendUp ? '' : 'rotate-180'
                          }`} />
                        {stat.trend}
                      </div>
                    </div>
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      ...
      <div className="flex-1 flex flex-col overflow-hidden lg:ml-0">
        ...
        <div className="flex-1 overflow-auto">
          {/* 🔔 Verification Banner */}
          {isVolunteer && !isVerified && activeTab !== "dashboard" && (
            <VerificationBanner
              onVerify={() => setActiveTab("dashboard")}
            />
          )}
